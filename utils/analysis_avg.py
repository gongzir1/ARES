...
def report(
    ...
    # mean
    dm = torch.as_tensor(server_payload["data"].mean, **setup)
    if dm.dim() == 0:  # scalar case
        dm = dm.unsqueeze(0)
    dm = dm[None, :, None, None]  # shape [1, C, 1, 1]

    # std
    ds = torch.as_tensor(server_payload["data"].std, **setup)
    if ds.dim() == 0:  # scalar case
        ds = ds.unsqueeze(0)
    ds = ds[None, :, None, None]  # shape [1, C, 1, 1]
    ...
    if order_batch:
        ...
        reconstructed_user_data["data"] = reconstructed_user_data["data"].cpu()[order.cpu()]
        if reconstructed_user_data["labels"] is not None:
            reconstructed_user_data["labels"] = reconstructed_user_data["labels"].cpu()[order.cpu()]
        rec_denormalized = rec_denormalized[order.cpu()]
    else:
        order = None
    true_user_data["labels"] = true_user_data["labels"].cpu()
    ...
    def preprocess_for_lpips(x):
        ...
        if x.size(1) == 1:
            x = x.repeat(1, 3, 1, 1)
        # resize to safe minimum
        x = F.interpolate(x, size=(32, 32), mode='bilinear', align_corners=False)
        return x

    rec_denormalized = preprocess_for_lpips(rec_denormalized)
    ground_truth_denormalized = preprocess_for_lpips(ground_truth_denormalized)
    ...
    for idx, payload in enumerate(server_payload["queries"]):
        ...
        with torch.no_grad():
            ...
            print('Reconstructed_user_data forward')
            feats_rec = model(reconstructed_user_data["data"].to(**setup))
            print('True_user_data forward')
...
def compute_batch_order(lpips_scorer, rec_denormalized, ground_truth_denormalized, setup):
    ...
    print("rec_denormalized shape:", rec_denormalized.shape)
    print("ground_truth_denormalized shape:", ground_truth_denormalized.shape)
    print("B:", B)
    ...
    with torch.inference_mode():
        ...
        for input in rec_denormalized:
            ...
            if input_scaled.size(1) == 1:
                input_scaled = input_scaled.repeat(1, 3, 1, 1)

            # resize to safe size
            input_scaled = F.interpolate(input_scaled, size=(32, 32), mode='bilinear', align_corners=False)

            # Forward through LPIPS
