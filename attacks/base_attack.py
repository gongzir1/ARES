...
class _BaseAttacker:
    ...
    def prepare_attack(self, server_payload, shared_data):
        ...
        # Convert mean
        dm = torch.as_tensor(server_payload["data"].mean, **self.setup)
        if dm.dim() == 0:  # scalar mean (e.g., MNIST)
            dm = dm.unsqueeze(0)
        self.dm = dm[None, :, None, None]  # shape [1, C, 1, 1]

        # Convert std
        ds = torch.as_tensor(server_payload["data"].std, **self.setup)
        if ds.dim() == 0:  # scalar std (e.g., MNIST)
            ds = ds.unsqueeze(0)
        self.ds = ds[None, :, None, None]  # shape [1, C, 1, 1]
